...
class Owner(commands.Cog):
    ...
    async def mail(self, ctx, *, user: BetterUserconverter = None):
        ...
        if user:
            ...
            embed_message.set_author(
                name=f"Mail from: {ctx.author}", icon_url=(ctx.author.avatar_url)
            )
        ...
        if await self.client.is_owner(ctx.author):
            ...
            guild_list = [
                ...
                % (
                    ...
                    g.name,
                    g.id,
                    (g.system_channel or g.text_channels[0]).mention,
                )
    ...
    @commands.command(
        brief="only works with JDJG, but this command is meant to send updates to my webhook"
    )
        ...
        if await self.client.is_owner(ctx.author):
            if args:
                if isinstance(ctx.channel, discord.TextChannel):
                    ...
                    embed.set_author(
                        name="JDJG's Update", icon_url="https://i.imgur.com/pdQkCBv.png"
                    )
    ...
    async def mutualguilds(self, ctx, *, user: BetterUserconverter = None):
        ...
        user_guildlist = [
            guild.name for guild in self.client.guilds if guild.get_member(user.id)
        ]
        send_list = ["Servers:"]
    ...
    async def addsus(self, ctx, *, user: BetterUserconverter = None):
        ...
        if user:
            ...
            await cur.execute(
                "INSERT INTO sus_users VALUES (?, ?)", (user.id, reason.content)
            )
    ...
    class SusUsersEmbed(menus.ListPageSource):
        async def format_page(self, menu, item):
            ...
            embed.add_field(
                name=f"User ID : {item[0]}",
                value=f"**Reason :** {item[1]}",
                inline=False,
            )
            return embed
    ...
    async def sus_users(self, ctx):
        ...
        menu = menus.MenuPages(
            self.SusUsersEmbed(sus_users, per_page=1), delete_message_after=True
        )
    ...
    async def command_bypass(
        self, ctx, user: BetterUserconverter = None, *, command=None
    ):
        ...
        if command:
            ...
            if command_wanted:
                await ctx.send(
                    f"{command_wanted.name} now accessible for the {user} for one command usage!"
                )
