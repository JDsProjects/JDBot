...
class Bot(commands.Cog):
    ...
    async def stats(self, ctx):
        ...
        embed.set_footer(
            text="if you at all don't get what this means, you can ask our support team, if you do understand you can ask for clarification"
        )
        await ctx.send(embed=embed)
    ...
    async def command_autocomplete(
        ...
        for command in self.bot.walk_commands():
            all_items.add(f"command:{command.qualified_name}")
    ...
    def process_source(self, src):
        ...
        try:
            ...
            filename = inspect.getsourcefile(type(src))
        try:
    ...
    @commands.command(brief="Gives the bot name")
    async def bot_introduction(self, ctx):
        await ctx.send("Hi My name Maria JDBot Jefferson.\nYou can call me Maria or JDBot \:)")
