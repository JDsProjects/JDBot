...
class Events(commands.Cog):
  ...
  async def on_message(self,message):
    ...
    if (isinstance(message.channel, discord.DMChannel)
        and (test.prefix is None or self.client.user.mentioned_in(message))
        and message.author.id != self.client.user.id and test.valid is False):
    ...
    if ((test.valid) == False and test.prefix != None
        and not self.client.user.mentioned_in(message) and test.command is None):
