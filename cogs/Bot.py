...
class Bot(commands.Cog):
  ...
  async def promises(self, ctx):
    ...
    embed.add_field(name = "1:", value = "We have a channel that logs when the bot joins or leaves a guild", inline = True)
    ...
    embed.add_field(name = "3:", value = "we will only store user ids(in the future for balance commands and economy commands(opt in only)", inline = True)
    embed.add_field(name = "4:", value = "We will store user id, and channel id(our channel), and last active, for ticket support if you need support. (is opt in)", inline = True)
    embed.add_field(name = "5:", value = "we Finally store invalid commands", inline = True)
    ...
    embed.add_field(name = "7", value = "You have all message content in global chat be logged to a private channel on discord for logs, channel ids, and guild ids will be logged for global chat, but otherwise, message content is not stored, except in moderation channels, and user ids are used to blacklist users per guild or globaly, or tell who is mod or staff or just a normal user.", inline = True)
