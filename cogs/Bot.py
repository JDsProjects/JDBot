...
class Bot(commands.Cog):
    ...
    async def invite(self, ctx):
        ...
        embed.add_field(name=f"{self.client.user.name} invite:",
        ...
        embed.add_field(name="Non Markdowned invite",
    ...
    async def owner(self, ctx):
        ...
        guilds_list = [guild for guild in self.client.guilds if guild.get_member(
            owner.id) and guild.get_member(ctx.author.id)]
        if not guilds_list:
            guild_list = "None"
        ...
        embed = discord.Embed(title=f"Bot Owner: {owner}", description=f"Type: {user_type}", color=random.randint(
        ...
        embed.add_field(name="Joined Discord: ", value=(
            owner.created_at.strftime('%m/%d/%Y %H:%M:%S')))
    ...
    async def open_source(self, ctx):
        embed = discord.Embed(title="Project at:\nhttps://github.com/JDJGInc/JDBot !",
                              description="you can also contact the owner if you want more info(by using the owner command) you can see who owns the bot.", color=random.randint(0, 16777215))
        embed.set_author(name=f"{self.client.user}'s source code:", icon_url=(
    ...
    async def promise(self, ctx):
        embed = discord.Embed(title="Basic rights that we guarantee",
                              description="The promises we make about our privacy policy", color=0xffb300)
        ...
        embed.add_field(
            name="\u2800", value="We will not nuke your servers, as this happened to us before and we absolutely hated it.", inline=True)
        await ctx.send(embed=embed)
        embedtwo = discord.Embed(title="Basic rights that we guarantee",
                                 description="The promises we make about our privacy policy", color=0xffb300)
        embedtwo.set_author(name="Page 2")
        embedtwo.add_field(
            name="\u2800", value="The bot has the ability to give you a list of people who are considered suspicious to us", inline=True)
        embedtwo.add_field(
            name="\u2800", value="The bot source code is open source and that allows you to make sure the bot is safe and secure and even host your own instance", inline=True)
        ...
        embedfinal = discord.Embed(title="Basic rights that we guarantee",
                                   description="The promises we make about our privacy policy", color=0xffb300)
        embedfinal.set_author(name="Page 3")
        embedfinal.add_field(
            name="\u2800", value="Using our bot to attempt to break TOS, will cause us to ban you from using the bot, and afterwars upgrade our security", inline=True)
    ...
    async def privacypolicy(self, ctx):
        embed = discord.Embed(title="Our privacy policy ",
                              description="This privacy policy applies to JDbot and other services ", color=0xffb300)
        embed.add_field(name="Our data request policy ",
                        value="If you wish to know what the bot knows about you please send a DM to the bot, and we will try to compile the data the bot may have on you.", inline=True)
        embed.add_field(name="The data we collect ",
                        value="We have a channel that logs when the bot joins or leaves a guild", inline=True)
        ...
        embedtwo = discord.Embed(
            title="Our privacy policy ", description="This privacy policy applies to JDbot and other services ", color=0xffb300)
        embedtwo.set_author(name="Page 2")
        embedtwo.add_field(name="Continuation of previous page",
                           value="We store the inputted invalid commands", inline=True)
        embedtwo.add_field(
            name="\u2800", value="We will in the future store user ids for balance commands and economy commands(opt in only) ", inline=True)
        embedtwo.add_field(
            name="\u2800", value="We currently store user ids, and channel ids(our channel), and last active channel, for ticket support if you will require support. (Opt in) ", inline=False)
        embedtwo.set_footer(
            text="For more info please contact me at JDJG Inc. Official#3493 ")
        await ctx.send(embed=embedtwo)
        ...
        embedfinal.add_field(
            name="\u2800", value="We also have a suspicious users list that is used to store a list of users who may cause problems on your servers and that you should probably avoid ", inline=True)
    ...
    async def support_invite(self, ctx):
        embed = discord.Embed(title="Consention requirement",
                              description="This command will send a discord invite in the channel you typed the command in", color=0x008080)
        embed.add_field(
            name="To consent", value="Type i agree in lower case into the channel", inline=True)
        await ctx.send(embed=embed)
