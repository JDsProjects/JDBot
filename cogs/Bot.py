...
class Bot(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        embed = discord.Embed(title="Bot Ping Data",
                              color=15428885, timestamp=ctx.message.created_at)

        embed.add_field(name="Bot Latency:",
                        value=f"{round((end - start)*1000)} MS", inline=False)

        embed.add_field(name="Websocket Response time:",
    ...
    async def invite(self, ctx):
        ...
        normal_inv_slash = discord.utils.oauth_url(self.bot.user.id, permissions=discord.Permissions(
            permissions=8), scopes=("bot", "applications.commands"))
        minimial_invite_slash = discord.utils.oauth_url(self.bot.user.id, permissions=discord.Permissions(
            permissions=70635073),  scopes=("bot", "applications.commands"))

        embed = discord.Embed(title="Invite link:",
                              color=random.randint(0, 16777215))
        embed.add_field(name=f"{self.bot.user.name} invite:",
                        value=f"[{self.bot.user.name} invite url]({normal_inv}) \nNon Markdowned invite : {normal_inv}")
        embed.add_field(name="Minimial permisions",
                        value=f"{ minimial_invite}")
        ...
        view.add_item(discord.ui.Button(
            label=f"{self.bot.user.name}'s Minimial Permisions Invite", url=minimial_invite, style=discord.ButtonStyle.link))

        view.add_item(discord.ui.Button(
            label=f"{self.bot.user.name}'s Normal Invite(Slash)", url=normal_inv_slash, style=discord.ButtonStyle.link, row=2))
        view.add_item(discord.ui.Button(label=f"{self.bot.user.name}'s Minimial Permisions(Slash)",
                      url=minimial_invite_slash, style=discord.ButtonStyle.link, row=2))
    ...
    async def owner(self, ctx):
        ...
        user_type = "Bot" if owner.bot else "User" if isinstance(
            owner, discord.User) else "Member"

        statuses = []

        badges = [utils.profile_converter(
            "badges", f) for f in owner.public_flags.all()] if owner.public_flags else []
        if owner.bot:
    ...
    async def team(self, ctx):
        ...
        embed.set_image(
            url=information.icon.url if information.icon else self.bot.display_avatar.url)
    ...
    async def stats(self, ctx):
        embed = discord.Embed(
            title="Bot stats", color=random.randint(0, 16777215))
        embed.add_field(name="Guild count", value=len(self.bot.guilds))
        embed.add_field(name="User Count:", value=len(self.bot.users))
        embed.add_field(name="True Command Count:",
                        value=f"{len(list(self.bot.walk_commands()))}")
        ...
        embed.add_field(name="Approximate Member Count:",
    ...
    async def promise(self, ctx):
        embed = discord.Embed(title="Promises we will follow:",
                              color=random.randint(0, 16777215))
        embed.add_field(
            name="Rule 1:", value="if you are worried about what the bot may collect, please send a DM to the bot, and we will try to compile the data the bot may have on you.")
        ...
        embed.add_field(
            name="Rule 3:", value="we will not nuke your servers, as this happened to us before and we absolutely hated it.")
        embed.add_field(
            name="Rule 4:", value="We will also give you a list of suspicious people")
        embed.add_field(
            name="Rule 5:", value="we also made sure our code is open source so you can see what it does.")
        embed.add_field(
            name="Rule 6:", value="We will also let you ask us questions directly, just DM me directly(the owner is listed in the owner command(and anyone should be able to friend me)")
        embed.add_field(
            name="Rule 7:", value="Using our bot to attempt to break TOS, will cause us to ban you from using the bot, then upgrade our security")
    ...
    async def promises(self, ctx):
        ...
        embed.add_field(
            name="3:", value="We store user ids for economy commands (Opt In)")
        embed.add_field(
            name="4:", value="We store user id , and support channel id, as well as last time it was used(for archive reasons), for allowing ticket based support. (Opt In)")
        ...
        embed.add_field(
            name="6.1:", value="This is a temp command, which is stored no where else, and we also delete the embed when done :D. If you have a problem with this contact me.")
        ...
        embed.add_field(
            name="8:", value="In the future we will store the guild id and channel id for member joining/leaving messages, and member changes")
        ...
        embed.add_field(name="Who Gets this info:",
        ...
        embed.add_field(name="More Info:",
                        value="Contact me at JDJG Inc. Official#3493")
        await ctx.send(embed=embed)
    ...
    async def support_invite(self, ctx):
        ...
        view = discord.ui.View()
        view.add_item(discord.ui.Button(label=f"Support Guild Invite",
                      url="https://discord.gg/sHUQCch", style=discord.ButtonStyle.link, row=1))
        ...
        if view.value is True:
            ...
            if isinstance(ctx.channel, discord.TextChannel):
                ...
                if not invite:
                    ...
                    embed = discord.Embed(
                        title=f"{ctx.author} wants your help", description=f"Invite: {invite.url} \nChannel : {ctx.channel.mention} \nName : {ctx.channel}", color=random.randint(0, 16777215))

                    embed.set_footer(
                        text=f"Guild: {ctx.guild} \nGuild ID: {ctx.guild.id}")
    ...
    async def apply_tester(self, ctx, *, reason=None):
        ...
        embed = discord.Embed(title=f"{ctx.author} requested to be a tester.",
                              description=f"For the reason of {reason}", color=random.randint(0, 16777215))
    ...
    async def currentprefix(self, ctx):
        embed = discord.Embed(
            title="Current Prefix:", description=f"{ctx.prefix}", color=random.randint(0, 16777215))
    ...
    async def uptime(self, ctx):
        ...
        embed.set_author(name=f"{self.bot.user}'s Uptime:",
    ...
    async def suggest(self, ctx, *, args=None):
        ...
        embed = discord.Embed(title=f"New Suggestion requested by {ctx.author}", description=f"Suggestion: {args}",
                              timestamp=ctx.message.created_at, color=random.randint(0, 16777215))
    ...
    async def support_dm(self, ctx, *, args=None):
        ...
        embed = discord.Embed(
            title=f"{args}", timestamp=ctx.message.created_at, color=random.randint(0, 16777215))

        embed.set_author(
            name=f"Help Needed from {ctx.author}:", icon_url=ctx.author.display_avatar.url)
    ...
    async def about(self, ctx):
        ...
        embed = discord.Embed(title="About Bot", description="Here you can view bot and author information",
                              timestamp=ctx.message.created_at, color=0xeb6d15)

        embed.add_field(name="Author Information",
                        value="```This Bot is made by JDJG Inc. Official#3493(you can find out more about owners from the owner command).```", inline=False)
        ...
        embed.add_field(name="Contributers",
                        value="```Shadi#9492 \nMiddlle#0101 \nDutchy#6127```")
