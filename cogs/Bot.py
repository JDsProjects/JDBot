...
class Bot(commands.Cog):
  ...
  async def promise(self,ctx):
    ...
    embed.add_field(name="_", value="If you wish to know what the bot knows about you please send a DM to the bot, and we will try to compile the data the bot may have on you.", inline=True)
    embed.add_field(name="_", value="For the safety of the bot w, we will be making sure the token is secure and making sure anyone who works on the project is very trustworthy.", inline=True)
    embed.add_field(name="_", value="We will not nuke your servers, as this happened to us before and we absolutely hated it.", inline=True)
    await ctx.send(embed=embed)
    embed=discord.Embed(title="Basic rights that we guarantee", description="The promises we make about our privacy policy", color=0xff8000)
    embedtwo.set_author(name="Page 2")
    embedtwo.add_field(name="_", value="The bot also has a feature to give you a list of suspicious people which shall never be removed", inline=True)
    embedtwo.add_field(name="_", value="The bot source code is open source and that allows you to make sure the bot is safe and secure and even host your own instance", inline=True)
    embedtwo.add_field(name="_", value="We also let you ask us questions directly, just DM me or any other maintainer directly (the owner is listed in the owner command(and anyone should be able to friend me or the maintaners)", inline=True)
    await ctx.send(embed=embedtwo)
    embedfinal=discord.Embed(title="Basic rights that we guarantee", description="The promises we make about our privacy policy", color=0xff8000)
    embedfinal.set_author(name="Page 3")
    embedfinal.add_field(name="_", value="Using our bot to attempt to break TOS, will cause us to ban you from using the bot, and afterwars upgrade our security", inline=True)
    embedfinal.add_field(name="_", value="Attempting to break discord TOS like having a giveaway but requiring people to join an external guild(will eventually be reportable to us, if the owner or the giveaway holder countinues then it should be reported to discord.(essentially breaking TOS using the functionalties our bot provides)", inline=True)
    embedfinal.add_field(name="_", value="If our bot doesn't do the giveaway requirements properly, as we don't enforce them, you can however report this to us, so we can contact the person abusing the bot to atempt to get them to stop, Thanks. if they don't listen we'll tell you, then you can report them.", inline=True)
