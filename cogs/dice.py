...
class Dice(commands.Cog):
    ...
    async def generate_embed(self, ctx, number):
        if str(number) is "20":
            url = "https://i.imgur.com/9dbBkqj.gif"
        if str(number) is "6":
            url = "https://i.imgur.com/6ul8ZGY.gif"
        else:
            url = "https://i.imgur.com/gaLM6AG.gif"
        ...
        embed.set_author(
            name=f"d{number} Rolled by {ctx.author}:", icon_url=(ctx.author.avatar_url)
        )
    ...
    @commands.command(
        brief="a command meant to flip coins", help="commands to flip coins, etc."
    )
    async def coin(self, ctx, *, args=None):
        if args:
            ...
            url_dic = {
                "heads": "https://i.imgur.com/MzdU5Z7.png",
            ...
            embed.add_field(
                name="The Coin Flipped: " + ("heads" if value else "tails"),
                value=f"You guessed: {args}",
            )
