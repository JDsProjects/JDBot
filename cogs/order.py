...
class Order(commands.Cog):
    ...
    async def order(self, ctx, *, args=None):
        ...
        if args:
            ...
            image_client = async_cse.Search(
                os.environ["image_api_key"], engine_id=os.environ["google_image_key"]
            )
            try:
                results = await image_client.search(
                    args, safesearch=True, image_search=True
                )
                emoji_image = sorted(
                    results,
                    key=lambda x: SequenceMatcher(None, x.image_url, args).ratio(),
                )[-1]
            ...
            embed.set_author(
                name=f"order for {ctx.author}:", icon_url=(ctx.author.avatar_url)
            )
            embed.add_field(
                name="Time Spent:", value=f"{int((time_after - time_before)*1000)}MS"
            )
    ...
    @commands.cooldown(1, 30, BucketType.user)
    @commands.command(
        brief="a command to shuffle images from google images",
        aliases=["order-shuffle"],
    )
    ...
    async def tenor(self, ctx, *, args=None):
        if args:
            results = await self.client.loop.run_in_executor(
                None, tenor_client.search(args, safesearch=True, limit=10)
            )
            print(results)
