...
class Moderation(commands.Cog):
    ...
    @commands.command(
        brief="a command to warn people, but if you aren't admin it doesn't penalize."
    )
    async def warn(self, ctx, Member: BetterMemberConverter = None):
        if warn_permission(ctx):
            ...
            if Member:
                ...
                embed.set_author(
                    name=f"You have been warned by {ctx.author}",
                    icon_url=("https://i.imgur.com/vkleJ9a.png"),
                )
    ...
    @commands.command(
        help="a command to scan for malicious bots, specificially ones that only give you random invites and are fake(work in progress)"
    )
    ...
    @commands.command(
        help="a way to report a user, who might appear in the sus list. also please provide ids and reasons. (work in progress"
    )
    async def report(self, ctx, *, args=None):
        if args:
            ...
            embed = discord.Embed(color=random.randint(0, 16777215))
            embed.set_author(
                name=f"Report by {ctx.author}", icon_url=(ctx.author.avatar_url)
            )
